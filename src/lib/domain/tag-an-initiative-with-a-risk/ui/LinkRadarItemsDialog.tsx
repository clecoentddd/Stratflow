// Moved from initiatives slice as part of vertical slice refactor
"use client";
  ...
  const handleToggle = (itemId: string) => {
    setSelectedIds((prev) => {
      const next = prev.includes(itemId)
        ...
        : [...prev, itemId];
      console.log('[LinkRadarItemsDialog] Toggled item:', itemId, 'Selected:', next);
      return next;
    });
  };

  const handleSave = () => {
    console.log('[LinkRadarItemsDialog] Saving selected radar item IDs:', selectedIds);
    try {
      onLinkItems(selectedIds);
      console.log('[LinkRadarItemsDialog] onLinkItems called successfully');
    } catch (err) {
      console.error('[LinkRadarItemsDialog] Error in onLinkItems:', err);
    }
    onOpenChange(false);
  };
