...
const applyEventsToTeam = (
  ...
  return events.reduce((team, event) => {
    ...
    switch (event.type) {
      ...
      // --- Radar Events ---
      case 'RadarItemCreated' as any:
        return { ...team, radar: [...(team.radar || []), (event as any).payload] };

      case 'RadarItemUpdated' as any:
        return {
          ...team,
          radar: (team.radar || []).map(item =>
            item.id === (event as any).payload.id ? { ...item, ...(event as any).payload } : item
          ),
        };

      case 'RadarItemDeleted' as any:
        return {
          ...team,
          radar: (team.radar || []).filter(item => item.id !== (event as any).payload.id),
        };

      // --- Strategy Events ---
      case 'StrategyCreated' as any:
        const newStrategy = {
          id: (event as any).payload.strategyId,
          description: (event as any).payload.description,
          timeframe: (event as any).payload.timeframe,
      ...
      case 'StrategyUpdated' as any:
        return {
          ...team,
          dashboard: {
            ...team.dashboard,
            strategies: team.dashboard.strategies.map(s =>
              s.id === (event as any).payload.strategyId ? { ...s, ...(event as any).payload } : s
            )
          }
        };

      // --- Initiative Events ---
      case 'InitiativeCreated' as any:
        return {
          ...team,
          dashboard: {
            ...team.dashboard,
            strategies: team.dashboard.strategies.map(s => {
              if (s.id !== (event as any).payload.strategyId) return s;
              const newInitiative = {
                id: (event as any).metadata?.initiativeId || 'unknown',
          ...
          dashboard: {
            ...
            strategies: team.dashboard.strategies.map(s => ({
              ...
              initiatives: s.initiatives.map(i =>
                i.id === (event as any).payload.initiativeId ? { ...i, name: (event as any).payload.name } : i
              )
            }))
          }
        };

      case 'InitiativeProgressUpdated' as any:
        return {
          ...team,
          dashboard: {
            ...team.dashboard,
            strategies: team.dashboard.strategies.map(s => ({
              ...s,
              initiatives: s.initiatives.map(i =>
                i.id === (event as any).metadata?.initiativeId ? { ...i, progression: (event as any).payload.progression } : i
              )
            }))
          }
        };

      case 'InitiativeDeleted' as any:
        return {
          ...team,
          dashboard: {
            ...team.dashboard,
            strategies: team.dashboard.strategies.map(s => {
              ...
              return {
                ...s,
                initiatives: s.initiatives.filter(i => i.id !== (event as any).payload.initiativeId)
              };
            })
          }
        };

      case 'InitiativeRadarItemsLinked' as any:
        return {
          ...team,
          dashboard: {
            ...team.dashboard,
            strategies: team.dashboard.strategies.map(s => ({
              ...s,
              initiatives: s.initiatives.map(i =>
                i.id === (event as any).payload.initiativeId ? { ...i, linkedRadarItemIds: (event as any).payload.linkedRadarItemIds } : i
              )
            }))
          }
        };
      ...
      case 'InitiativeItemCreated' as any:
        return {
          ...team,
          dashboard: {
            ...team.dashboard,
            strategies: team.dashboard.strategies.map(s => ({
              ...s,
              initiatives: s.initiatives.map(i => {
                if (i.id !== (event as any).payload.initiativeId) return i;
                const newItem = {
                  id: (event as any).payload.itemId,
                  text: (event as any).payload.text,
                  status: (event as any).payload.status ?? 'todo',
                };

                return {
                  ...i,
                  steps: i.steps.map(step =>
                    step.key === (event as any).payload.stepKey
      ...
      case 'InitiativeItemUpdated' as any:
        return {
          ...team,
          dashboard: {
            ...team.dashboard,
              ...
              initiatives: s.initiatives.map(i => ({
                ...
                steps: i.steps.map(step => ({
                  ...
                  items: step.items.map(item =>
                    item.id === (event as any).metadata?.itemId ? { ...item, text: (event as any).payload.text } : item
                  )
                }))
              }))
            }))
      ...
      case 'InitiativeItemDeleted' as any:
        return {
          ...team,
          dashboard: {
            ...team.dashboard,
            strategies: team.dashboard.strategies.map(s => ({
              ...s,
              initiatives: s.initiatives.map(i => {
                if (i.id !== (event as any).metadata?.initiativeId) return i;
                return {
                  ...i,
                  steps: i.steps.map(step => ({
                    ...step,
                    items: step.items.filter(item => item.id !== (event as any).metadata?.itemId)
                  }))
                };
              })
            }))
...
// Generic handler for ANY team-modifying event - invalidates cache
function onTeamModifyingEvent(event: any) {
  console.log(`🔄 [LIVE-PROJECTION] Invaliding teams cache due to ${event.type} on ${event.aggregateId}`);
  setTeamsProjectionCache(null);
  setTeamsCacheExplicitlyEmptied(false);
}
...
registerProjectionHandler('TeamUpdated', onTeamModifyingEvent);
registerProjectionHandler('RadarItemCreated', onTeamModifyingEvent);
registerProjectionHandler('RadarItemUpdated', onTeamModifyingEvent);
registerProjectionHandler('RadarItemDeleted', onTeamModifyingEvent);
registerProjectionHandler('StrategyCreated', onTeamModifyingEvent);
registerProjectionHandler('StrategyUpdated', onTeamModifyingEvent);
registerProjectionHandler('InitiativeCreated', onTeamModifyingEvent);
registerProjectionHandler('InitiativeUpdated', onTeamModifyingEvent);
registerProjectionHandler('InitiativeDeleted', onTeamModifyingEvent);
registerProjectionHandler('InitiativeProgressUpdated', onTeamModifyingEvent);
registerProjectionHandler('InitiativeRadarItemsLinked', onTeamModifyingEvent);
registerProjectionHandler('InitiativeItemCreated', onTeamModifyingEvent);
registerProjectionHandler('InitiativeItemUpdated', onTeamModifyingEvent);
registerProjectionHandler('InitiativeItemDeleted', onTeamModifyingEvent);
