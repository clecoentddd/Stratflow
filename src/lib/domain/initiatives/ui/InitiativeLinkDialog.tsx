...
export function InitiativeLinkDialog({
  ...
  useEffect(() => {
    ...
    const fetchItems = async () => {
      ...
      try {
        ...
        console.log('[LinkDialog] fetching initiatives', { url: `/api/initiatives/search?${params.toString()}`, sourceInitiativeId });
        const res = await fetch(`/api/initiatives/search?${params.toString()}`, { signal: controller.signal });
        const data = await res.json();
        console.log('[LinkDialog] fetch result', { count: Array.isArray(data) ? data.length : 0, sample: Array.isArray(data) ? data.slice(0,3) : [] });
        setItems(Array.isArray(data) ? data.filter((x) => x.initiativeId !== sourceInitiativeId) : []);
      } catch (e) {
        console.warn('[LinkDialog] fetch failed', e);
        setItems([]);
      } finally {
        setLoading(false);
      }
  ...
  const handleLink = async () => {
    ...
    console.log('[LinkDialog] linking', { sourceInitiativeId, targetsCount: targets.length, targets });
    ...
    const data = await res.json();
    console.log('[LinkDialog] link response', { ok: res.ok, data });
    if (res.ok) {
