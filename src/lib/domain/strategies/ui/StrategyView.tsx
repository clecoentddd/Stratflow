...
export function StrategyView({
  ...
  const handleUpdateStrategy = useCallback(async (updatedValues: Partial<Strategy>) => {
    ...
    try {
      ...
      if (!response.ok) {
        const msg = json.error?.message || json.error || 'Failed to update strategy';
        throw new Error(msg);
      }
  ...
  const handleCreateInitiative = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const msg = json.error?.message || json.error || 'Failed to create initiative.';
        throw new Error(msg);
      }
    ...
    fetch(`/api/initiatives?teamId=${orgId}`, {
      ...
      .then(async res => {
        if (!res.ok) {
          ...
          const msg = json.error?.message || json.error || 'Failed to delete initiative.';
          throw new Error(msg);
        }
      ...
      <div className={styles.strategyHeader}>
        <div className={styles.compactHeaderRow}>
          <div className={styles.leftGroup}>
            ...
            <div className={styles.titleInfo}>
              ...
              <div className={styles.metaInfo}>
                <span className={styles.metaBadge} title="Timeframe">
                  <Milestone className="w-3 h-3" />
                  {strategy.timeframe}
                </span>
                <span className={styles.metaBadge} title="Total Initiatives">
          ...
          <div className={styles.rightGroup}>
            <div className={styles.headerProgressArea}>
              <div className={styles.progressLabelRow}>
                <span className={styles.progressValue}>{overallProgression}% Done</span>
              </div>
              <div className={styles.slimProgressBarContainer}>
                <div
