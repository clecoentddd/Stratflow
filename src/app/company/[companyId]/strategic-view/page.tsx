...
export default async function StrategicViewPage({
    params,
    searchParams
}: {
    ...
    const all = queryEligibleInitiatives({ states: ['Draft', 'Active'], companyId });
    const selectedId = id && all.find(r => r.id === id) ? id : undefined;
    const { nodes, edges } = buildSubgraph(selectedId, companyId);

    return (
        <main style={{ padding: '1rem' }}>
            <h1 style={{ fontSize: '1.5rem', fontWeight: 600, marginBottom: '1rem' }}>Strategic View - {companyId.replace('company-', '')}</h1>
